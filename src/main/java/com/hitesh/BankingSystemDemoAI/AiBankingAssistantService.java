...
public class AiBankingAssistantService {
    ...
    // Constructor injection for both our AI model and H2 database repository
    ...
    public String chatWithCustomerRAG(Long customerId, String userMessage) {
        // 1. RETRIEVAL: Java fetches live data from the H2 relational database
        BankAccount account = accountRepository.findById(customerId)
                .orElse(null);
        ...
        // 2. AUGMENTATION: We inject the live database values directly into our system prompt string
        String fullPromptText = """
            ...
            INSTRUCTION: Use the database context above to answer the user's question directly, accurately, and politely. Keep your answer to a short, friendly paragraph.
            """.formatted(account.getUser(), account.getBalance(), account.getType(), userMessage);

        // 3. GENERATION: Hand the augmented prompt to Llama 3.2 to compile the natural response
